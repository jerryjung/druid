...
public class JDBCIndexTaskClientTest extends EasyMockSupport
{
  ...
  private JDBCIndexTaskClient client;

  public JDBCIndexTaskClientTest(int numThreads)
  {
  ...
  public void setUp() throws Exception
  {
    ...
    client = new TestableJDBCIndexTaskClient(httpClient, objectMapper, taskInfoProvider);
  ...
  public void testNoTaskLocation() throws Exception
  {
    ...
    Assert.assertEquals(JDBCIndexTask.Status.NOT_STARTED, client.getStatus(TEST_ID));
...
//    Assert.assertEquals(false, client.setEndOffsets(TEST_ID, ImmutableMap.<Integer, Long>of(), true));
  ...
  @Test(expected = JDBCIndexTaskClient.TaskNotRunnableException.class)
...
//    Assert.assertEquals(10, (long) results.get(1));
  ...
  {
    client = new TestableJDBCIndexTaskClient(httpClient, objectMapper, taskInfoProvider, 3);
...
//    Assert.assertEquals(10, (long) results.get(1));
  ...
  {
    client = new TestableJDBCIndexTaskClient(httpClient, objectMapper, taskInfoProvider, 2);
  ...
  {
    JDBCIndexTask.Status status = JDBCIndexTask.Status.READING;
    ...
    JDBCIndexTask.Status results = client.getStatus(TEST_ID);
    verifyAll();
...
//  }
  ...
  {
    public TestableJDBCIndexTaskClient(
